...
`tasks` or `tasks -v`. You can also have a  look at [Command-Line-Reference] 
for a discussion on built-in tasks commonly used at the command line.  

A key will be printed in the resulting list if:

* it's built-in sbt (like `name` in the examples shown above)
